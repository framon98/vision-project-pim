function output_image = Regla_5(input_image)
...
Cb_temp = 0;
Cr_temp = 0;
Image_dimension = fil*col;

for x = 1:fil
    for y = 1:col
        Y_temp  = imagen_Y(x, y)  + Y_temp;
        Cb_temp = imagen_Cb(x, y) + Cb_temp;
        Cr_temp = imagen_Cr(x, y) + Cr_temp;
    end
end

Y_mean  = (Y_temp/Image_dimension);
Cb_mean = (Cb_temp/Image_dimension);
Cr_mean = (Cr_temp/Image_dimension);

for x = 1:fil
    for y = 1:col
        if imagen_Y(x, y) >= Y_mean  &&  imagen_Cb(x, y) <= Cb_mean  &&  imagen_Cr(x, y) >= Cr_mean
            output_image(x, y) = 1;
        else
            output_image(x, y) = 0;
        end
...
% figure('Name','Imagen obtenida mediante regla 5','NumberTitle','off');
% imshow(output_image)
